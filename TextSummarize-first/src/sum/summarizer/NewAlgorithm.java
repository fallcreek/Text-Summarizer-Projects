...
public class NewAlgorithm implements Summarizer{
	/*
	...
	public int M = 2; //maximum amount of parameter-type sentences.
	
	public ArrayList<Sentence> generateSummary(ArrayList<Sentence> WaitingList, int length, int lengthType)
	{
//		for(Sentence s: WaitingList)
//			System.out.println(s.getText() + " Score: " + s.getSentenceScore());
		
		//modify by lizhengyao 0310 
		for(int i = 0; i < WaitingList.size(); i++)
		{
			if(WaitingList.get(i).getSentenceScore() <= 0)
			{
		...
		for(int i = 0; i < WaitingList.size(); i++)
		{
			if(WaitingList.get(i).getNumWords() < 8)
			{
				WaitingList.remove(i);
				i--;
			}
...
//			modify by lizhengyao 0407 删除了判断句子是否小于L的条件
			...
			//add parameter-type sentences' amount constraint
			setZero(row);
			for(int i = 0; i < numSentence; i++)
			{
				if(WaitingList.get(i).getSentenceType() == 1)
				{
					row[i+1] = 1;
				}
			}
			lp.addConstraint(row, LpSolve.LE, M);
			...
			for(int i = 0; i < numSentence; i++)
			{
				row[i+1] = WaitingList.get(i).getSentenceScore();
			}
